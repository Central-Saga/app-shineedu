"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { useBreadcrumbStore } from "@/shared/infrastructure/store/breadcrumb.store";
import { PageHeader } from "@/shared/presentation/components/PageHeader";
import { usePermissionGuard } from "@/shared/presentation/hooks/usePermissionGuard";
import { DataTableToolbar } from "@/shared/presentation/components/table/DataTableToolbar";
import { DataTablePagination } from "@/shared/presentation/components/table/DataTablePagination";
import { useDebouncedValue } from "@/shared/presentation/hooks/useDebouncedValue";
import { authStore } from "@/modules/auth/infrastructure/auth.store";
import { StatsCard } from "@/shared/presentation/components/StatsCard";
import { Card, CardContent } from "@/components/ui/card";
import { Label } from "@/components/ui/label";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Button } from "@/components/ui/button";
import { Plus, CheckCircle, XCircle, ClipboardList } from "lucide-react";
import { toast } from "sonner";
import Link from "next/link";
import {
  listJobVacancies,
  deleteJobVacancy,
} from "@/modules/job-vacancy/infrastructure/job-vacancy.repository";
import { JobVacancyTable } from "@/modules/job-vacancy/presentation/components/JobVacancyTable";
import type { JobVacancy } from "@/modules/job-vacancy/domain/entities";

const STATUS_OPTIONS = [
  { label: "Semua", value: "__all__" },
  { label: "Aktif", value: "1" },
  { label: "Nonaktif", value: "0" },
];

const SORT_OPTIONS = [
  { label: "Tanggal", value: "created_at" },
  { label: "Judul", value: "title" },
  { label: "Lokasi", value: "location" },
] as const;

export default function JobVacanciesPage() {
  const { allowed } = usePermissionGuard("job_vacancy.view");
  const router = useRouter();
  const { setItems } = useBreadcrumbStore();

  const [items, setTableItems] = useState<JobVacancy[]>([]);
  const [loading, setLoading] = useState(true);
  const [meta, setMeta] = useState({
    current_page: 1,
    per_page: 15,
    total: 0,
    last_page: 1,
    from: null as number | null,
    to: null as number | null,
  });

  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(15);
  const [search, setSearch] = useState("");
  const debouncedQ = useDebouncedValue(search, 400);
  const [activeFilter, setActiveFilter] = useState<string | null>(null);
  const [sortKey, setSortKey] = useState<string>("created_at");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");

  const [stats, setStats] = useState({
    total: 0,
    aktif: 0,
    nonaktif: 0,
  });

  const canCreate = authStore.hasPermission("job_vacancy.create");
  const canUpdate = authStore.hasPermission("job_vacancy.update");
  const canDelete = authStore.hasPermission("job_vacancy.delete");

  useEffect(() => {
    setItems([
      { label: "Dashboard", href: "/dashboard" },
      { label: "Lowongan Kerja" },
    ]);
  }, [setItems]);

  useEffect(() => {
    if (!allowed) return;
    loadData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allowed, page, perPage, debouncedQ, activeFilter, sortKey, sortDir]);

  async function loadData() {
    setLoading(true);
    try {
      const params = {
        page,
        per_page: perPage,
        q: debouncedQ || undefined,
        is_active: activeFilter === "__all__" || activeFilter === null ? undefined : activeFilter,
        sort_by: sortKey,
        sort_dir: sortDir,
      };
      const { items: resItems, meta: resMeta } = await listJobVacancies(params);
      setTableItems(resItems);
      setMeta(resMeta);

      const [totalRes, aktifRes, nonaktifRes] = await Promise.all([
        listJobVacancies({ per_page: 1 }),
        listJobVacancies({ per_page: 1, is_active: "1" }),
        listJobVacancies({ per_page: 1, is_active: "0" }),
      ]);
      setStats({
        total: totalRes.meta.total,
        aktif: aktifRes.meta.total,
        nonaktif: nonaktifRes.meta.total,
      });
    } catch (e) {
      toast.error("Gagal memuat data lowongan");
    } finally {
      setLoading(false);
    }
  }

  async function handleDelete(item: JobVacancy) {
    try {
      await deleteJobVacancy(item.id);
      toast.success("Lowongan dihapus");
      loadData();
    } catch (e) {
      toast.error("Gagal menghapus lowongan");
    }
  }

  if (!allowed) return null;

  return (
    <div>
      <PageHeader
        title="Lowongan Kerja"
        description="Kelola lowongan kerja (job vacancies) untuk landing & form lamaran"
        actions={
          canCreate ? (
            <Button asChild>
              <Link href="/job-vacancies/new">
                <Plus className="mr-2 size-4" /> Tambah Lowongan
              </Link>
            </Button>
          ) : undefined
        }
      />

      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatsCard
          label="Total"
          value={stats.total}
          icon={ClipboardList}
          variant="primary"
          description="Semua lowongan"
        />
        <StatsCard
          label="Aktif"
          value={stats.aktif}
          icon={CheckCircle}
          variant="success"
          description="Lowongan terbuka"
        />
        <StatsCard
          label="Nonaktif"
          value={stats.nonaktif}
          icon={XCircle}
          variant="danger"
          description="Lowongan ditutup"
        />
      </div>

      <Card className="rounded-2xl shadow-sm">
        <CardContent className="space-y-4 pt-6">
          <DataTableToolbar
            searchValue={search}
            onSearchChange={setSearch}
            searchPlaceholder="Cari judul, lokasi..."
            filters={[
              {
                key: "status",
                label: "Status",
                options: STATUS_OPTIONS,
                value: activeFilter,
                onChange: (v) => {
                  setActiveFilter(v);
                  setPage(1);
                },
              },
            ]}
            sort={{
              value: sortKey,
              options: [...SORT_OPTIONS],
              onChange: (v) => setSortKey(v),
              direction: sortDir,
              onToggleDirection: () =>
                setSortDir((d) => (d === "asc" ? "desc" : "asc")),
              defaultValue: "created_at",
              defaultDirection: "desc",
              onDirectionChange: setSortDir,
            }}
          />

          <div className="flex items-center gap-2">
            <Label className="text-muted-foreground whitespace-nowrap text-sm">
              Per halaman
            </Label>
            <Select
              value={String(perPage)}
              onValueChange={(v) => {
                setPerPage(Number(v));
                setPage(1);
              }}
            >
              <SelectTrigger className="h-9 w-[100px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="15">15</SelectItem>
                <SelectItem value="30">30</SelectItem>
                <SelectItem value="50">50</SelectItem>
                <SelectItem value="100">100</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <JobVacancyTable
            items={items}
            loading={loading}
            onView={(item) => router.push(`/job-vacancies/${item.id}`)}
            onEdit={(item) => router.push(`/job-vacancies/${item.id}/edit`)}
            onDelete={handleDelete}
            canUpdate={canUpdate}
            canDelete={canDelete}
          />

          <DataTablePagination meta={meta} onPageChange={setPage} />
        </CardContent>
      </Card>
    </div>
  );
}
